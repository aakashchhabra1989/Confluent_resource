# AWS Topics Module
# This module contains AWS-specific Kafka topic definitions

# Create a Kafka Topic (optional) using Admin API Key for each sub-environment
resource "confluent_kafka_topic" "aws_dummy_topic" {
...
resource "confluent_kafka_topic" "aws_dummy_topic_with_schema" {
  for_each = toset(var.sub_environments)
